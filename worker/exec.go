...
// ExecPackage is a helper function to execute Vela pipelines
// that are sent to the Worker via the Server.
//
//nolint:nilerr // ignore returning nil - don't want to crash worker
func (w *Worker) ExecPackage(index int, pkg *Package) error {
	// invalid pkg
	if pkg == nil {
		return nil
	}
